...
const FilterSlector = () => {
  ...
  const rangeMove = ()=> {
    ...
    const target = originalContent.results.filter(item => 
                ...
                >
                  {
                    genreList.map((item,idx) => {
                      return (
                        <Dropdown.Item key={item.name} tabIndex={item.id} eventKey={idx}>{item.name}</Dropdown.Item>
                      )
                    })
                  }
                </DropdownButton>
      ...
      <div className='allShow'>
          ...
          <div className='range_box'>
            <div className='range_result'>
              <h5>From: <span>{minNum}</span> - To: <span>{maxNum}</span></h5>
            </div>
